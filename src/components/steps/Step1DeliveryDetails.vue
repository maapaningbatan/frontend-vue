...
// Convert suppliers & warehouses to SelectSearch options
...
const formattedPurchaseDate = computed(() => {
  if (!props.deliveryData.purchase_date) return null
  try {
    return new Date(props.deliveryData.purchase_date).toLocaleDateString()
  } catch {
    return props.deliveryData.purchase_date
  }
})

const formattedPrDate = computed(() => {
  if (!props.deliveryData.pr_date) return null
  try {
    return new Date(props.deliveryData.pr_date).toLocaleDateString()
  } catch {
    return props.deliveryData.pr_date
  }
})
</script>
    ...
    <div>
      ...
      <form class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <Input v-model="props.deliveryData.iar_number" type="text" placeholder="Enter IAR Number"
            class="mt-1 w-full h-11 border border-gray-300 rounded-lg bg-gray-50 shadow-sm focus:ring-1 focus:ring-blue-400 focus:border-blue-500 text-sm text-gray-700" />
        </div>

        <!-- Supplier -->
        <div>
          <Label class="block text-sm font-medium text-gray-700">Supplier</Label>
          <SelectSearch v-model="props.deliveryData.supplier" :options="suppliersOptions" placeholder="Select Supplier"
            class="mt-1 w-full h-11 border border-gray-300 rounded-lg bg-gray-50 shadow-sm focus:ring-1 focus:ring-blue-400 focus:border-blue-500 text-sm text-gray-700" />
        </div>
        ...
        <div>
          ...
          <Input v-model="props.deliveryData.purchase_order_number" type="text"
        ...
        <div>
          ...
          <DatePicker v-model="props.deliveryData.purchase_date" placeholder="Select Purchase Date"
            class="mt-1 w-full h-11 border border-gray-300 rounded-lg bg-gray-50 shadow-sm focus:ring-1 focus:ring-blue-400 focus:border-blue-500 text-sm text-gray-700" />
        </div>
        ...
        <div>
          ...
          <Input v-model="props.deliveryData.receiving_office"
            class="mt-1 w-full h-11 border border-gray-300 rounded-lg bg-gray-100 shadow-sm text-sm text-gray-700 dark:bg-gray-800 dark:text-white"
            readonly />
        </div>
        ...
        <div>
          ...
          <SelectSearch v-model="props.deliveryData.warehouse" :options="warehousesOptions"
        ...
        <div>
          ...
          <Input v-model="props.deliveryData.code_number"
            class="mt-1 w-full h-11 border border-gray-300 rounded-lg bg-gray-100 shadow-sm text-sm text-gray-700 dark:bg-gray-800 dark:text-white"
        ...
        <div>
          ...
          <div
            class="mt-1 w-full h-11 px-3 flex items-center border border-gray-300 rounded-lg bg-gray-100 text-sm text-gray-700 shadow-sm dark:bg-gray-800 dark:text-white">
            {{ user?.First_Name
        ...
        <div class="md:col-span-2">
          ...
          <TextArea id="purpose" placeholder="Purpose" v-model="props.deliveryData.purpose" :rows="3"
